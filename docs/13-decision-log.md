...
| D32 | Analytics | **Vercel Web Analytics** | Changed 2026-08-16 from Cloudflare Web Analytics, per direction. Cookieless, so still no cookie banner (which itself protects conversion). Added as a script tag, not the `@vercel/analytics` package — that package is a framework component wrapper around the same file, and installing it would break the zero-dependency build (D-zero-dep). **Known limit:** the Hobby plan includes 2,500 events/month, well under the 40k monthly visitors doc 12 §6 targets. Revisit at launch: either upgrade, or add Cloudflare Web Analytics alongside (a host-independent script tag, free and unlimited) for the headline counts. |
